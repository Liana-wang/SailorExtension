 ...
 */
export type Matcher = (key: string, replacements?: Record<string, any>) => string;
 ...
 */
export const i18nFactory = ({ translations, locale = translations[0] }: Settings): I18N => {
    ...
    const i18n: I18N = (resources: ReadonlyArray<ReadonlyArray<string>> = []) => {
        ...
        const findMatch = (key: string): string => {
            ...
            return typeof result !== undefined ? result : (match[KeyIndex] || '') // 返回匹配项或Config.key对应的语言
        }
